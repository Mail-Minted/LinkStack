...
<div class="flex align-items-center list-user-action">
  ...
  @php
    // Impersonation is a POST now (it used to be a GET, i.e. CSRF-able).
    // The old markup emitted the href only when allowed and rendered a
    // dead grey button otherwise; a disabled submit button is the same
    // thing and keeps the SVG below in one place.
    $mmCanImpersonate = !$user->adminUser
        && Auth::user()->id !== $user->id
        && $user->block !== 'yes'
        && ($user->email_verified_at != '' || env('REGISTER_AUTH') == 'auth');

    // Built here rather than with @if inside the tag: Blade directives in a
    // component tag's attributes are parsed by the component compiler before
    // the directive compiler runs, which leaves the @endif orphaned.
    $mmImpersonateStyle = $mmCanImpersonate
        ? 'background:#3a57e8;border-color:#3a57e8;'
        : 'background:#6c757d;border-color:#6c757d;';
  @endphp
  <x-post-action :action="route('authAsID', $user->id)" class="btn btn-sm btn-icon btn-primary" :style="$mmImpersonateStyle" data-bs-toggle="tooltip" data-bs-placement="top" data-original-title="{{__('messages.tt.Impersonate')}}" :disabled="!$mmCanImpersonate" aria-label="Impersonate" data-bs-original-title="Impersonate">
    <span class="btn-inner">   
       <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
